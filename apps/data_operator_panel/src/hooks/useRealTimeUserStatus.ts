...
// User type for data operator panel
interface User {
  id: string
  email: string
  role: string
...
export const useRealTimeUserStatus = (): UseRealTimeUserStatusReturn => {
  ...
  const isValidDataOperator = useCallback((user: User | null): boolean => {
    return !!(
      user &&
      user.role === 'data_operator' &&
      user.is_verified === true &&
      user.group === 'internalusers'
    )
  }, [])
  ...
  const transformToUser = useCallback((profile: UserProfile): User => ({
    ...
    email_verified: profile.email_confirmed_at ? true : false,
  ...
  }, [authUser, transformToUser, logout])
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      .channel(`data_operator_status_${authUser.id}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
  ...
  return {
    ...
    loading,
    error,
    isValidDataOperator: isValidDataOperator(currentUser),
